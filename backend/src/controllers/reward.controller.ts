...
export const getClaimableRewards = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const allRewards = await Reward.find({}).lean();

    // Filter rewards user can claim (has enough XP and hasn't claimed)
    const claimableRewards = allRewards.filter(reward =>
      user.xp >= reward.xpCost && !reward.claimedBy.some((claimedUserId) => claimedUserId.toString() === userId)
    );
...
export const claimReward = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const user = await User.findById(userId).select("xp level name");
    if (!user) {
      res.status(404).json({ message: "User not found" });
      return;
    }
    ...
    // Deduct XP cost
    await awardXP(userId, -reward.xpCost);
...
const createClaimNotification = async (userId: string, rewardName: string) => {
  try {
