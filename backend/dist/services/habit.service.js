...
exports.completeHabit = exports.getHabitStreak = exports.deleteHabit = exports.updateHabit = exports.getHabits = exports.createHabit = void 0;
...
const calculateHabitStreak = (habit, _currentTimezone = 'Etc/UTC') => {
...
const completeHabit = async (habitId, userId, timezone) => {
    const habit = await habit_model_1.Habit.findOne({ _id: habitId, user: userId });
    if (!habit) {
        throw new Error("Habit not found or unauthorized");
    }
    ...
    const alreadyCompletedToday = habit.completedDates.some((date) => new Date(date).toDateString() === today);
    ...
    await (0, xp_service_1.awardXP)(userId, habit.xpReward || 10);
...
const checkIfHabitIsDueToday = (habit, today) => {
    ...
    if (startDateStart && todayStart < startDateStart)
        return false;
    if (endDateStart && todayStart > endDateStart)
        return false;
    // Check frequency
    if (habit.frequency === 'daily') {
